...
class Orders extends CI_Model
{
	...
	function getOrder($data = null){
		...
		else
		{
			...
			$this->db->select('cliId, cliNombre, cliApellido, cliDocumento, cliDomicilio');
			...
			if ($query->num_rows() != 0)
			{
				...
				if($action == 'EntrM'){
					...
					$this->db->select('ordendetrabajodetalle.*');
					$this->db->from('ordendetrabajodetalle');
					$this->db->where(array('ordId'=>$id));
					$query = $this->db->get();
	...
	function printRemite($data = null){
		...
		else
		{
			...
			$this->db->select('ordendetrabajodetalle.*, remitodetalle.*');
