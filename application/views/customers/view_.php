...
<div class="row">
	<div class="col-xs-12">
      ...
      <div class="row">
        ...
        <div class="col-xs-5">
          <select class="form-control" id="fisId"  <?php echo ($data['read'] == true ? 'disabled="disabled"' : '');?> >
            <?php 
              foreach($data['fiscal'] as $f){
                echo '<option value="'.$f['fisId'].'" '.($data['customer']['fisId'] == $f['fisId'] ? 'selected' : '').'>'.$f['fisDescripcion'].'</option>';
              }
            ?>
          </select>
        </div>
      ...
      <div class="row">
        <div class="col-xs-4">
          <label style="margin-top: 7px;">Tipo: </label>
        </div>
        <div class="col-xs-5">
          <select class="form-control" id="cliTipo"  <?php echo ($data['read'] == true ? 'disabled="disabled"' : '');?> >
              <?php 
                  echo '<option value="BL" '.($data['customer']['cliTipo'] == 'BL' ? 'selected' : '').'>BL</option>';
                  echo '<option value="NG" '.($data['customer']['cliTipo'] == 'NG' ? 'selected' : '').'>NG</option>';
              ?>
          </select>
        </div>
      </div><br>